...
const OnlineDonationForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/generate-payment-link`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          external_id: slugify("online-donation-" + Date.now() + "-" + formData.fullName + "-" + formData.email + "-" + formData.phone + "-" + amount), 
