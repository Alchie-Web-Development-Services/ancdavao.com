...
const PledgeContent = () => {
  const daysLeft =
    userPledge.timeframe -
    Math.floor(userPledge.progress * userPledge.timeframe);
  const urgentDeadline = new Date(
    userPledge.urgent.deadline,
  ).toLocaleDateString();
    ...
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="mb-2 text-lg font-semibold">
          {userPledge.currency}
          {userPledge.amount} pledged in {userPledge.timeframe} days
        </div>
        <div className="mb-2 text-sm text-gray-600">
          You have fulfilled {userPledge.currency}
          {userPledge.fulfilled} so far.
        </div>
        <PledgeProgressBar progress={userPledge.progress} />
        <div className="text-xs text-gray-500 mb-2">
          {Math.round(userPledge.progress * 100)}% complete — {daysLeft} days
          left
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <div className="mb-2 text-sm">
          <span className="font-bold">
            {userPledge.urgent.percentFunded}% funded
          </span>{" "}
          — only {userPledge.currency}
          {userPledge.urgent.amountNeeded} more needed this week (by{" "}
          {urgentDeadline})
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <div className="text-sm mb-2">
          ...
          <span className="font-bold">{userPledge.impact.kidsFed} kid(s)</span>{" "}
          for{" "}
          <span className="font-bold">{userPledge.impact.weeks} week(s)</span>{" "}
          so far!
        </div>
